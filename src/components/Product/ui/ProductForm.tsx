  ...
  function ProductForm(props, ref) {
    ...
    const onSubmit = async (formInputs: ProductFormAdd) => {
      ...
      if (isEdit && product) {
        ...
        if (photos?.length && addedProductId) {
          const photosResponce = await uploadPhotos(
            photosFormData,
            addedProductId
          )
          messages.push(photosResponce)
        }

        if (preview) {
          const previewFormData = new FormData()
          previewFormData.append("preview", preview[0])

          const previewResponce = await uploadPreviewPhoto(
            previewFormData,
            addedProductId
          )

          messages.push(previewResponce)
        }
